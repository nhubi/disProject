...
the weights and, with appropriate extensions and taking into account the noise, this method suits perfectly the 
robot swarm problem. 
A key part of PSO is the choice of a good fitness function to evaluate the performance of each
simulation. A good reference fitness function is the one
...
\ref{sec:3}, along with the PSO implementation. 
...
R and minimum (or maximum) if it is higher than S. Between R and S we usually adopt a linear behaviour, even if more complex equations can be considered.

To compute all the necessary information, the robots can communicate
with a supervisor that sends them their own position (not the ones of the other robots) and the goal position. They can also communicate with the other robots which are not too far away in order to exchange their relative position. They moreover use their infrared (\textbf{TODO: check if true})
sensors to detect obstacles.

\subsection{Move to goal}  
The simplest motor schema is the \textit{move to goal} one. Knowing
the positions of the robots and the position of the goal from the supervisor, this schema generates a speed vector with a direction from the unit center of the robots to the goal and of norm:
\begin{equation}
...
space that have to be computed at each time step and which corresponds to the position in which the robot should be in order to stay in formation. Knowing the position of the goal, the unit center of
the formation, and the coordinates of each robot relatively to the unit center, it is possible to compute the vector from the actual position to the expected
...
To detect the other robots, we can either use the built-in sensors, or local communication. We decided to use the second one since we found them a lot more precise and less noisy than sensor values. Once implemented local communications, the robots can easily receive the
...
The e-puck sensors are quite noisy and we need to take this into account when dealing with obstacle detection. There are eight sensors on an e-puck and we know the position and thus the orientation of each of them. Now, $R$ and $S$ are sensor values: if the received sensor value is below $R$, we assume that no obstacle is near; if it is over $S$, we generate a normalized vector in the opposite
direction. In between, we found out that a linear function does not
provide any correct behaviour. We thus decided to give a ``premium'' (\textbf{TODO: to explain better}) to
