...
\subsection{Set-ups}
Three worlds have been defined, as seen in Figure \textbf{TODO}, in order to try to find parameters that best allow obstacle avoidance:
\begin{itemize}
\item[-] a random world, where a goal position is defined, the four robots are randomly positioned in a certain area, and six obstacles are positioned in an area between the goal and the robots;
\item[-] a world with a wall of obstacles between a predefined goal and the four robots which are randomly positioned behind the wall;
\item[-] a world with an obstacle configuration which is hard to escape for a robot that get stuck against it. 
\end{itemize}
\subsection{Tests of the motor-schemes}
We can notice that a good feeling allowed us to find good manually tuned parameters quite easily. Those parameters are reported in Table \ref{manual_param}. 
\begin{table}
\begin{center}
\begin{tabular}{|c|c|c|c|}
  ...
  \hline
  $move\_to\_goal$ &  $0.07$ & $0.1$ & $0.5$\\
  $keep\_formation$ & $0.35$ & $0.03$ & $0.1$\\
  $avoid\_robot$ & $0.07$ & $0.05$ & $0.1$\\
