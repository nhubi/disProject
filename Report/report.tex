...
We compared the resulting fitness values between three different world set-
ups and evaluated the quality and the convergence of the \textit{PSO} 
results. 
...
the four other ones since we normalized them so that the sum is a unit.
 We gave to the other parameters the values manually found and reported in Table \ref{manual_param}.
...
In Figure \ref{graphs} are reported for each world the best global fitness acquired so far at each iteration (in blue) and the mean fitness values of the particles at each iteration with their standard deviation (in red). 
\begin{figure*}[!t]
\centerline{
...
The final performance is similar in the three cases, even if the value is a bit lower in the hard configuration case. While a certain convergence can be observed well in the third case, it is less clear in the two other worlds. This might come from the fact that in the first world, the probability is high to reach the goal even with a bad parameter set, and on the other side, in the second one, it is too hard. That is why the standard deviation stays high over the ten iterations, while in the last worlds, it shrinks drastically. We would need more iterations and more particles to reach a better convergence in all cases. 
...
time and thus to have a bigger swarm size and more \textit{PSO} iterations. Also running each of the three worlds per evaluation and generate a combined fitness would be possible.
