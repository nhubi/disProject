...
difficult and time expensive to optimally tune the parameters specifying the
schemas. The introduction of machine learning techniques thus becomes necessary.

In this project we have implemented a homogeneous, motor-schema based robot 
controller and tuned it applying solution-sharing Particle Swarm 
Optimization (\textit{PSO}) using a group fitness function. 

We compared the resulting fitness values between three different world set-ups and evaluated the quality and the convergence of the \textit{PSO} 
results. 
...
good explanation of the advantages of motor-schema based
controllers over traditional and less flexible ones. 

One of the main challenges that one may encounter while implementing a 
controller is to achieve multiple different objectives at the same time
to obtain an optimized and good looking walk of the robots. 
...
we have studied the influence on the simulation of Particle Swarm Optimization (\textit{PSO}), a
...
simulations and the final section \ref{sec:5} will be reserved to our conclusions and possible future work. \\
...
global speed vector which will determine the wheel speeds of the robots and thus their next position. 
...
\caption{Computation of the expected position in the \textit{keep formation} motor schema: wedge formation}
...
According to \cite{IEEEhowto:balch_behaviour_based}, unit based formations lead to better results than leader referenced formations, this is why the former has been implemented. Four formations can be chosen by the user, as showed in Figure \ref{img:formation}, by passing a value as an argument to the world: \textit{line} ($0$), \textit{column} ($1$), \textit{wedge} ($2$) and \textit{diamond} ($3$). 
...
positions of the other robots. For each robot, a speed vector is generated, pointing outward the other robot and having the following norm:
\begin{equation*}
...
In 1995, Eberhart and Kennedy  \cite{IEEEhowto:first_pso} proposed a new method called \textit{Particle Swarm Optimization}, which evolved from the intent to simulate the movements of bird flocks and fish schools, to efficiently solve this kind of problems. 
...
optimize, i.e. each parameter is a dimension in the `particle space'. 
...
getting stuck in a local minimum does decrease. For the results not to be biased, the swarm 
size should be greater or equal to the dimensionality $D$. Due to simulation problems with 
Webots\textregistered, we had to set the number of particles to the smallest possible number. 

In our implementation, we have decided to normalize the ranges of each dimension in the `particle 
space'. Usually, not all ranges of parameters to 
optimize are in the same order of magnitude. For example e-puck's distance sensors return 
values in the range $[0,4096]$, whereas the communication range of a robot is limited to a few 
meters. Optimizing certain thresholds would return values in the according ranges. Now, a 
velocity limit of $1$ does not have the same meaning at all in both dimensions. Whilst in the 
communication range direction, a particle could cross the whole space in only one iteration 
when moving at full speed, it would need many steps to reach the border in the sensor 
...
\hfill
\subsection{OCBA}
Since the first \textit{PSO} version in 1995, much research has been done to improve the algorithm's performance. One of the most recent extensions is called \textit{PSO OCBA} and was proposed by Di Mario, Navarro and Martinoli in 2015 \cite{IEEEhowto:martinoli_pso_noise_2}. 

The problem of the initial \textit{PSO} algorithm is that it does not deal with randomness. When it comes to determine controller parameters, this is a big issue since in this case the measured performances of several evaluations with same parameters usually have a big variance. This is because there are many sources of randomness, such as noisy sensor measurements or moving obstacles (for example, other robots). The problem is that, therefore, a bad choice of parameters can result in a good measured performance and conversely. We can thus not be sure if the particle position with the best evaluation is actually the optimal one.
...
\section{Results}
\label{sec:4}
\IEEEPARstart{T}{he} well functioning of the implementation of the motor-
...
Furthermore, we have experimented some issues with Webots\textregistered. After a 
few simulated hours, sensors showed strange values or robots stopped even 
if the control panel showed still turning wheels. Consequently, we decided to 
reduce the number of parameters to be optimized to four: the weights of the 
motor-schemas $A$, $B$, $C$ and $D$. Note that the last weight, 
corresponding to the random walk motor-schema, is uniquely determined from 
the four other ones since we normalized them so that the sum equals to one.
 We gave to the other parameters the values manually found and reported in Table \ref{manual_param}, which seemed to give quite good results.
...
supervisor to send the positions of the obstacles to the robots, and for 
...
\begin{tabular}{|c|c|c|c|c|}
  ...
  $random\_walk$ & $0.04$ & $0.06$ & $0.05$ & $0.26$\\
  \hline
\end{tabular}
\caption{Values of the weights found by PSO} \label{weights}
\end{center} 
\end{table}

We can first observe that the motor-schema \textit{random\_walk} gets a higher value when we use sensors to detect obstacles. It could seem strange since sensors are already intrinsically noisy, but the explanation comes from the fact that sensors detect the obstacles later and thus need a higher random factor to get away from them when \textit{avoid\_obstacles} is not sufficient. 

We can also notice that \textit{keep\_formation} and \textit{avoid\_obstacles} get always a slightly higher weight, as we guessed when we manually tuned the parameters. Almost all the weights found are quite similar between the different worlds, which shows a certain consistency in the results. 

In Figure \ref{graphs} are reported for each world the best global fitness acquired so far at each iteration (in blue) and the mean fitness values of the particles at each iteration with their standard deviations (in red). 
\begin{figure*}[!t]
\centerline{
\subfloat[Random world]{\includegraphics[width=2.5in]{Images/randomWorldGraph.jpg} \label{graph_random}}
\hfill
\subfloat[World with a wall of obstacles]{\includegraphics[width=2.5in]{Images/wallWorldGraph.jpg} \label{graph_wall}}
\hfill
\subfloat[Difficult obstacle configuration]{\includegraphics[width=2.5in]{Images/difficultWorldGraph.jpg}\label{graph_hard}}}
\caption{Fitness values in different worlds}
\label{graphs}
\end{figure*}

The final performance is similar in the three cases, even if the value is a bit lower in the hard configuration case. While a certain convergence can be observed well in the third case, it is less clear in the two other worlds. This might come from the fact that in the first world, the probability to reach the goal is high even with a bad set of parameters, and on the other side, in the second one, it is too hard. That is why the standard deviation stays high over the ten iterations, while in the last worlds, it shrinks drastically. We would need more iterations and more particles to reach a better convergence in all cases, and to perform a better analysis on the results. 


\section{Conclusion}
\label{sec:5}

\IEEEPARstart{W}{e} have implemented a motor-schema based e-puck controller 
that allows the robot to approach a given goal, avoiding obstacles, while being in a user chosen 
formation with other robots without colliding. In order to tune the 
...
showed bizarre and physically impossible behaviours after several hours of 
simulated time.
...
can say that we are on a good path with our approach.

As a future work, we propose to implement \textit{PSO} in a webot-external program, 
...
could avoid the current situation, where come robots' crashes after a several 
hours of simulated time. This would allow us to run \textit{PSO} for longer 
time and thus to have a bigger swarm size and more \textit{PSO} iterations, getting more accurate and better results. Also running each of the three worlds per evaluation and generate a combined fitness would be possible.
